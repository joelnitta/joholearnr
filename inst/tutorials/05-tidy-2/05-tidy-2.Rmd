...
```{r europe-pipeline-check}
  ...
  grade_this({
    ...
    fail_if_not_equal(
      # (?s) turns on "dot-all" mode, allowing .* to match newlines and
      # any characters.
      x = str_detect(user_code_text, "(?s)filter.*mutate.*arrange"),
      y = TRUE,
      message = "パイプラインの順序が正しくありません、または、必要なステップが含まれていない可能性があります"
    )
    fail_if_not_equal(
      x = inherits(.result, "data.frame"),
      y = TRUE,
...
グループ毎にデータの集計を行うためには、グループ化する必要があります。

- Q5: `group_by()`を使って、大陸毎のグループを作ってください。詳細については[スライド](https://joelnitta.github.io/joho-shori/day5/#/%E3%82%B0%E3%83%AB%E3%83%BC%E3%83%97%E5%8C%96)を参照してください。
...
グループ分けができたら、次に`summarize()`でグループ毎の集計ができます。詳細については[スライド](https://joelnitta.github.io/joho-shori/day5/#/0/44)を参照してください。
...
- `mean()`：平均値を計算する
- `sd()`：標準偏差を計算する
- `n()`：それぞれのグループ毎のデータの数（行数）を数える

- Q6: 大陸毎に一人当たりの国内総生産の平均を`mean_gdp_percap`という列を持って計算してください。
...
`n()`はグループ毎のデータ数を数える関数ですが、`min()`や`max()`などとの使い方が少し異なります。

`min()`や`max()`には列名を指定しないといけません（例えば、`min(life_exp)`）が、`n()`は括弧の中に何も書く必要がありません。

例えば、大陸毎のデータ数を確認しましょう：
...
これはどういうことか、一つの国だけに絞るもっと分かりやすくなります：
...
- Q7: 大陸毎に一人当たりの国内総生産の平均とを`mean_gdp_percap`という列で、それぞれの大陸のデータの数を`n_obs`という列で計算してください。
