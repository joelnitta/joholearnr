...
今までは、数字しか扱って来なかったが、Rには他のベクトルの種類がある。Rでは、ベクトルの種類を**「型」**（type）と呼ぶ。他の種類のベクトルも作り方は数字ベクトルと同じです:`c()`を使う。
...
**注意：** 文字ベクトルを作るとき、データ（値）を**引用符**（`"`）で囲まないといけない。そうしないと、Rはそれが他のオブジェクトの名前だと思ってしまい、エラーが出る。
...
なので、異なる型のデータを結合させようとすると、自動的に同じ型に変換されます。
...
question("一つ選んでください：",
  ...
  answer("文字", message = "文字ベクトルの要素は引用符が付いている"),
  answer("数字", correct = TRUE),
  answer("句読点", message = "Rに「句読点」というデータ型が存在しない"),
...
でも、今までは`x = なんちゃら`というふうに指定しなかったけど、ちゃんと切り捨てることができましたよね。なぜでしょう？

Rでは、引数の位置も決まっています。

1. `round()`の**１つ目**の引数は**切り捨てる数字**です
2.  `round()`の**2つ目**の引数は**残す少数の数**です

従って、Rでは引数名を指定しない場合、**位置で引数を感知する**。

###

- Q9: `round()`の引数名`x`と`digits`を使って、`length_in`の**少数を一つ残して**、切り捨ててください。
...
```{r round-no-args-names, exercise = TRUE, exercise.lines = 2, exercise.setup = "make-length-cm-solution"}
...
`seq()`は数字ベクトルを作る関数です。`seq()`の引数は以下のとおりです：
...
Rには非常にたくさんの関数があります。どんなにRをマスターしても、全部を覚えることはまずないし、必要でもありません。

ある関数についてその使い方（引数）を確認したい時は、`?関数名`、あるいは`help(関数名)`を実行する。

- `toupper()`の関数の名前はどれでしょうか？`?`あるいは`help()`を使って、RStudioで確認してください。

```{r argument-name, echo=FALSE}
question("`toupper()`の関数の名前は...",
...
Rには備え付けの関数がたくさんあり、この関数だけによるRの使い方を**「Base R」**（基本R）と呼ぶ。

でも、やりたいことによってはBase Rでは足りない時もある。Base Rの機能を拡張するものを**「パッケージ」**（Package）と呼ぶ。パッケージとは、何かの共通テーマを持った関数のまとまりです。以下はパッケージの例です：

- 図表を書く`ggplot2`
- エクセルファイルを読み込む`readxl`
- Rから電子メールを送る`gmailr`

要するに、スマートフォンのアプリと同じようなものです。現在、Rには2万以上のパッケージがあり、情報処理に関することならほぼ何でもできる。

### パッケージのインストール

パッケージを使いたい場合、まずはそのパッケージをダウンロードする必要がある。
...
- Q12: `praise`パッケージをインストールしてください（出力が出るまで数秒がかかるかもしれません）
...
パッケージをインストール（ダウンロード）するのと、そのパッケージを使える状態にするのは、別のことです。

パッケージを使える状態にする状態にする関数は`library()`です（自分のパソコンが図書館だとしたら、それぞれのパッケージが本になる、みたいなイメージです）。

`install.packages()`と二つの大きな違いがあります：

1.`install.packages()`は一個のパッケージについて一回だけすればいいですが、** `library()`はRを使う度に必要です**。
1.`install.packages()`を使う時はパッケージ名を引用符で囲むが、** `library()`は引用符でパッケージ名を書かない**。
...
`library()`を実行した後、Rは特に何かのメッセージを返さない。これはつまり、そのパッケージを使える状態に成功したという意味です（うまく行かなかったら、エラーが出る）。

- Q13: `praise`パッケージをロードしてください。
...
- Q15: `praise()`関数の出力はどんな型ですか？

```{r praise-output-type, echo = FALSE}
question("一つ選んでください：",
  answer("論理", message = "論理ベクトルはTRUEかFALSE"),
  answer("文字", correct = TRUE,),
  answer("数字", message = "数字に引用符が付いていない"),
  answer("句読点", message = "Rに「句読点」というデータ型が存在しない"),
  random_answer_order = TRUE,
  allow_retry = TRUE
)
