...
関数の書き方は関数名をまず書いて、その後に括弧の中に入力を書きます：`関数名(入力)`
...
でも、`round()`を最初に使った時は`x = なんちゃら`というふうに指定しなかったけど、ちゃんと切り捨てることができましたよね。なぜでしょう？

Rでは、引数の位置も決まっています。

1. `round()`の**１つ目**の引数は**切り捨てる数字**です（`x`）
...
- Q9: `round()`の引数名`x`と`digits`を使って、`length_in`の少数を一つ残して、切り捨ててください。
...
- Q12 `toupper()`の引数の名前はどれでしょうか？ `?` あるいは`help()`を使って、RStudioで確認してください。
...
- Q13: `praise`パッケージをインストールしてください（出力が出るまで少し時間がかかるかもしれません）
...
2. `install.packages()`を使う時はパッケージ名を引用符で囲むが、**`library()`は引用符でパッケージ名を書かない。**
