...
- Q2: `arrange()`関数を使って、`gapminder`データを寿命（短いから長い方へ）と並び替えてください。
  - （この質問以下からでは、結果は新しいオブジェクトとして保存する必要はありませんが、コードの出力はデータフレームにしてください）
...
- Q5: `filter()`関数を使って、`gapminder`から日本（"Japan"）のデータのみを抽出してください。
  - `filter()`に使う条件の記号については、[講義のスライド](https://joelnitta.github.io/joho-shori/day4/#/%E3%83%87%E3%83%BC%E3%82%BF%E3%81%AE%E6%AF%94%E8%BC%83)を参考にしてください。
...
`filter()`に`OR`（「もしくは」）という条件をかけたい時は、`|`を使います。例えば、大陸がアフリカまたはアジアに絞る：
